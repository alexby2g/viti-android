...
class _ClientModuleScreenState extends State<ClientModuleScreen> {
  ...
  Future<void> _newRequest() async {
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Nueva solicitud ${created['codigo'] ?? ''} creada.'),
        ),
      );
      await _load();
    } on ApiException catch (exception) {
  ...
  Widget _home() {
    ...
    final latestRequest = requests.isEmpty
        ? <String, dynamic>{}
        : requests.first;
      ...
      child: ListView(
        ...
        children: [
          VitiPageHeader(
            ...
            subtitle:
                'Tu empresa, solicitudes, proyecto y aplicaciones en una sola ruta. VITI te muestra dónde estás y cuál es el siguiente paso.',
            actions: [
          ...
          Wrap(
            ...
            children: [
              ...
              VitiMetricTile(
                label: 'Proyecto',
                value: project.isEmpty
                    ? 'Sin proyecto'
                    : '${project['progreso'] ?? 0}%',
                icon: Icons.account_tree_outlined,
                tone: VitiTone.primary,
                onTap: () => widget.onNavigate('proyecto'),
              ),
              VitiMetricTile(
                label: 'Aplicaciones',
                value: '${apps.length}',
            ...
            builder: (context, constraints) {
              ...
              final requestPanel = latestRequest.isEmpty
                  ? VitiEmptyState(
                      ...
                      message:
                          'Una nueva idea genera su propia SOL y queda separada de solicitudes anteriores.',
              ...
              final projectPanel = project.isEmpty
                  ? const VitiEmptyState(
                      ...
                      message:
                          'Cuando una solicitud sea aprobada y convertida, el progreso aparecerá aquí.',
                      icon: Icons.account_tree_outlined,
                    )
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    Text(
                      '${_text(client['telefono'], 'Sin teléfono')} · ${_text(client['ci'], 'CI no registrado')}',
                      style: TextStyle(
          ...
          final actions = Wrap(
            ...
            children: [
              ...
              OutlinedButton.icon(
                ...
                label: const Text('Escribir a VITI'),
              ),
              OutlinedButton.icon(
  ...
  Widget _nextStep(
    ...
    if (request.isEmpty) {
      ...
      message =
          'Describe la necesidad de tu empresa. Cada solicitud mantiene su propio historial.';
      icon = Icons.assignment_add;
      tone = VitiTone.info;
      action = _newRequest;
      ...
      message =
          'La SOL todavía está en borrador. Completa la información para que AGR Studio pueda revisarla.';
      ...
      message =
          'Puedes seguir el estado y comunicarte por el buzón mientras se define alcance y propuesta.';
      icon = Icons.manage_search;
      tone = VitiTone.primary;
      action = () => widget.onNavigate('mensajes');
      actionLabel = 'Abrir buzón';
    } else if (apps.any(
      (app) => app['puede_usar'] == true && app['acceso_cliente'] == true,
    )) {
      title = 'Tu aplicación está disponible';
      message =
          'Ya puedes entrar al sistema entregado y trabajar con la información de tu empresa.';
      ...
      message =
          'Revisa el progreso, avances visibles y fechas del proyecto activo.';
      icon = Icons.terminal;
      tone = VitiTone.primary;
      ...
      child: ListView(
        ...
        children: [
          VitiPageHeader(
            ...
            subtitle:
                'Cada nueva necesidad genera una SOL independiente; las anteriores permanecen como historial.',
            actions: [
              FilledButton.icon(
                onPressed: _newRequest,
                icon: const Icon(Icons.add),
          ...
          if (requests.isEmpty)
            VitiEmptyState(
              ...
              message:
                  'Crea una solicitud para iniciar un nuevo proceso con AGR Studio.',
              icon: Icons.assignment_add,
              action: FilledButton.icon(
                onPressed: _newRequest,
                icon: const Icon(Icons.add),
  ...
  Widget _requestRow(Map<String, dynamic> request) {
    ...
    return VitiEntityRow(
      title:
          '${_text(request['codigo'], 'SOL')} · ${_text(request['titulo'], 'Solicitud de sistema')}',
      subtitle: _text(
        request['empresa_nombre'],
        _text(
          _map(request['empresa'])['nombre_comercial'],
          'Empresa por definir',
        ),
      ),
      ...
      badges: [
        ...
        if (request['prioridad'] != null)
          VitiStatusBadge(
            'Prioridad ${vitiPretty('${request['prioridad']}')}',
            tone: VitiTone.neutral,
          ),
      ],
  ...
  Widget _requestPanel(Map<String, dynamic> request) {
    ...
    return VitiInspector(
      title:
          '${_text(request['codigo'], 'SOL')} · ${_text(request['titulo'], 'Solicitud')}',
      ...
      children: [
        VitiKeyValue(
          ...
          _text(
            request['empresa_nombre'],
            _text(_map(request['empresa'])['nombre_comercial'], 'Sin empresa'),
          ),
          icon: Icons.business_outlined,
        ),
        VitiKeyValue(
          'Prioridad',
          vitiPretty('${request['prioridad'] ?? 'normal'}'),
      ...
      child: ListView(
        ...
        children: [
          VitiPageHeader(
            title: 'Mi proyecto',
            subtitle:
                'Progreso técnico, fase actual, aplicación y avances visibles de tu proyecto.',
            actions: [
              IconButton.filledTonal(
                onPressed: _load,
                tooltip: 'Actualizar',
          ...
          if (project.isEmpty)
            const VitiEmptyState(
              title: 'Todavía no tienes un proyecto activo',
              message:
                  'Cuando una solicitud aprobada se convierta en proyecto, aparecerá aquí.',
              icon: Icons.account_tree_outlined,
            ),
      ...
      child: Column(
        ...
        children: [
            ...
            children: [
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    Text(
                      '${_text(project['codigo'], 'PRO')} · ${_text(project['nombre'], 'Proyecto VITI')}',
                      style: const TextStyle(
                        fontSize: 18,
                        fontWeight: FontWeight.w900,
                      ),
          ...
          if (!compact) ...[
            ...
            if (updates.isEmpty)
              Text(
                'Todavía no hay avances visibles para tu empresa.',
                style: TextStyle(
                  color: Theme.of(context).colorScheme.onSurfaceVariant,
                ),
              ),
            for (final update in updates.take(6))
              ListTile(
                contentPadding: EdgeInsets.zero,
                leading: const Icon(Icons.check_circle_outline, size: 20),
                title: Text(
                  _text(update['titulo'], 'Actualización'),
                  style: const TextStyle(fontWeight: FontWeight.w800),
                ),
                subtitle: Text(_text(update['descripcion'], 'Sin descripción')),
              ),
          ],
        ],
      ),
      ...
      child: ListView(
        ...
        children: [
          VitiPageHeader(
            ...
            subtitle:
                'Tus sistemas conectados a VITI, con estado de acceso y operación claramente separados.',
            actions: [
              IconButton.filledTonal(
                onPressed: _load,
                tooltip: 'Actualizar',
          ...
          if (apps.isEmpty)
            const VitiEmptyState(
              ...
              message:
                  'Cuando un proyecto sea entregado, su aplicación aparecerá aquí.',
              icon: Icons.apps_outlined,
            ),
          LayoutBuilder(
            builder: (context, constraints) {
              ...
              return Wrap(
                ...
                children: [
                  for (final app in apps)
                    SizedBox(width: width, child: _appCard(app)),
                ],
              );
            },
          ),
        ...
        children: [
          Row(
            children: [
              Container(
                ...
                child: Icon(
                  key == 'electrofrio' ? Icons.ac_unit : Icons.apps_outlined,
                ),
              ),
              const SizedBox(width: 12),
              Expanded(
          ...
          Wrap(
            ...
            children: [
              ...
              VitiStatusBadge(
                canUse ? 'Acceso habilitado' : 'Sin acceso',
                tone: canUse ? VitiTone.success : VitiTone.warning,
              ),
            ],
          ),
          if (_text(app['estado_mensaje'], '').isNotEmpty) ...[
            const SizedBox(height: 12),
            Text(
              _text(app['estado_mensaje'], ''),
              style: TextStyle(
                fontSize: 12,
                color: Theme.of(context).colorScheme.onSurfaceVariant,
                height: 1.35,
          ...
          Row(
            children: [
              Expanded(
                child: Text(
                  canUse && native
                      ? 'Lista para trabajar desde VITI'
                      : native
                      ? 'Integración nativa disponible'
                      : 'Controlada desde VITI',
                  style: TextStyle(
                    fontSize: 12,
              ...
              if (canUse && native)
                FilledButton.icon(
                  onPressed: () => _openApp(app, key, canUse, native),
  ...
  Future<void> _openApp(
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            _text(
              app['estado_mensaje'],
              'Esta aplicación todavía no está disponible.',
            ),
          ),
        ),
      );
    ...
    if (!native) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text(
            'Esta aplicación todavía se está adaptando a la versión nativa.',
          ),
        ),
      );
      return;
              ...
              children: [
                Wrap(
                  ...
                  children: [
                    ...
                    VitiStatusBadge(
                      'Prioridad ${vitiPretty('${request['prioridad'] ?? 'normal'}')}',
                      tone: VitiTone.neutral,
                    ),
                  ],
                ),
                ...
                if (_text(request['descripcion'], '').isNotEmpty)
                  VitiKeyValue(
                    'Descripción',
                    _text(request['descripcion'], ''),
                    icon: Icons.notes,
                  ),
              ],
            ),
