...
class _PaymentModuleScreenState extends State<PaymentModuleScreen> {
  ...
  Future<void> _load() async {
    ...
    try {
      data = widget.admin
          ? await widget.repository.adminBilling()
          : await widget.repository.clientBilling();
      if (widget.admin && selectedProof != null) {
        final proofs = _collectProofs(
          _items(data['proyectos']),
          _items(data['suscripciones']),
        );
        final selectedId = _int(_map(selectedProof!['pago'])['id']);
        selectedProof = proofs
            .where(
              (item) =>
                  _int(_map(item['pago'])['id']) == selectedId &&
                  _text(item['tipo'], '') == _text(selectedProof!['tipo'], ''),
            )
            .firstOrNull;
      }
    ...
    if (file.size > 5 * 1024 * 1024) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('El comprobante no puede superar 5 MB.'),
          ),
        );
      }
      return null;
  ...
  Future<void> _sendProjectProof(Map<String, dynamic> project) async {
    ...
    try {
      ...
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Comprobante enviado para revisión.')),
      );
      await _load();
    } on ApiException catch (exception) {
      if (mounted) {
  ...
  Future<void> _sendSubscriptionProof(Map<String, dynamic> project) async {
    ...
    if ((double.tryParse(amount) ?? 0) <= 0 || _int(subscription['id']) <= 0) {
      return;
    }
    final file = await _pickProof();
    if (file == null || !mounted) return;
    ...
    try {
      ...
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Comprobante de suscripción enviado.')),
      );
      await _load();
    } on ApiException catch (exception) {
      if (mounted) {
      ...
      child: ListView(
        ...
        children: [
          VitiPageHeader(
            title: 'Mis pagos',
            subtitle:
                'Desarrollo y suscripción permanecen separados. Tu comprobante conserva trazabilidad y el saldo cambia únicamente después de la revisión de AGR Studio.',
            actions: [
          ...
          if (projects.isEmpty)
            const VitiEmptyState(
              title: 'Sin obligaciones de pago',
              message:
                  'Cuando exista un proyecto o suscripción con saldo, aparecerá aquí.',
              icon: Icons.payments_outlined,
            ),
          for (final project in projects) ...[
            _clientProjectPayment(project),
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                Text(
                  '${_text(config['banco'], 'Medio de pago VITI')} · Titular ${_text(config['titular'], 'AGR Studio')}',
                  style: TextStyle(
  ...
  Widget _clientProjectPayment(Map<String, dynamic> project) {
    ...
    final ratio = agreed <= 0
        ? 0.0
        : (paid / agreed).clamp(0.0, 1.0).toDouble();
    return VitiPanel(
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
            ...
            children: [
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    Text(
                      '${_text(project['codigo'], 'PRO')} · ${_text(project['nombre'], 'Proyecto')}',
          ...
          Wrap(
            ...
            children: [
              VitiKeyValue(
                'Acordado',
                _money(project['precio_acordado']),
                icon: Icons.request_quote_outlined,
              ),
              VitiKeyValue(
                'Pagado',
                _money(project['pagado']),
                icon: Icons.check_circle_outline,
              ),
              VitiKeyValue(
                'Pendiente',
                _money(project['pendiente']),
          ...
          Wrap(
            ...
            runSpacing: 8,
            children: [
              if (next['puede_enviar'] == true)
                FilledButton.icon(
                  onPressed: sending ? null : () => _sendProjectProof(project),
                  icon: const Icon(Icons.upload_file),
                  label: Text('Enviar comprobante · ${_money(next['monto'])}'),
                ),
              if (next['comprobante_pendiente_id'] != null)
                const VitiStatusBadge(
          ...
          if (subscription.isNotEmpty) ...[
            ...
            Row(
              children: [
                ...
                Expanded(
                  child: Text(
                    'Suscripción · ${_text(subscription['plan'], 'Plan VITI')}',
                    style: const TextStyle(fontWeight: FontWeight.w900),
                  ),
                ),
                VitiStatusBadge(
                  vitiPretty('${subscription['estado'] ?? ''}'),
                  tone: vitiToneForStatus('${subscription['estado'] ?? ''}'),
                ),
              ],
            ),
            const SizedBox(height: 8),
            Text(
              'Pendiente ${_money(subscription['importe_pendiente'])}',
              style: TextStyle(
                color: Theme.of(context).colorScheme.onSurfaceVariant,
              ),
            ),
            ...
            Wrap(
              ...
              runSpacing: 8,
              children: [
                if (subscription['puede_enviar_comprobante'] == true)
                  OutlinedButton.icon(
                    onPressed: sending
                        ? null
                        : () => _sendSubscriptionProof(project),
                    icon: const Icon(Icons.upload_file),
                    label: const Text('Enviar comprobante de suscripción'),
                  ),
                if (subscription['comprobante_pendiente_id'] != null)
                  const VitiStatusBadge(
  ...
  Widget _adminExperience() {
    ...
    final visible = allProofs
        .where((wrapper) {
          ...
          final haystack =
              '${company['nombre_comercial']} ${payer['nombre']} ${payer['usuario']} ${parent['codigo']} ${parent['nombre']} ${payment['comprobante_nombre']} ${payment['estado_revision']}'
                  .toLowerCase();
          return haystack.contains(query.toLowerCase().trim());
        })
        .toList(growable: false);
    final pending = allProofs
        .where(
          (item) =>
              _text(_map(item['pago'])['estado_revision'], '') ==
              'pendiente_revision',
        )
        .length;
        ...
        final header = Column(
          ...
          children: [
            VitiPageHeader(
              title: 'Pagos VITI',
              subtitle:
                  'Control económico, comprobantes y saldos con revisión antes de afectar los importes.',
              actions: [
                IconButton.filledTonal(
                  onPressed: _load,
                  tooltip: 'Actualizar',
            ...
            Wrap(
              ...
              children: [
                VitiMetricTile(
                  label: 'Por cobrar',
                  value: _money(summary['por_cobrar_proyectos']),
                  icon: Icons.account_balance_wallet_outlined,
                  tone: VitiTone.warning,
                ),
                VitiMetricTile(
                ...
                VitiMetricTile(
                  ...
                  value: '${summary['suscripciones_activas'] ?? 0}',
                  icon: Icons.autorenew,
                  tone: VitiTone.success,
                ),
                VitiMetricTile(
                  label: 'Recurrente mensual',
                  value: _money(summary['ingreso_recurrente_mensual']),
            ...
            VitiSearchField(
              ...
              hint: 'Buscar comprobante, empresa, pagador o proyecto…',
          ...
          child: visible.isEmpty
              ...
              : ListView.builder(
                  ...
                  itemBuilder: (context, index) =>
                      _proofRow(visible[index], desktop: desktop),
                ),
        );
                    ...
                    Expanded(
                      ...
                      child: selectedProof == null
                          ? const VitiEmptyState(
                              ...
                              message:
                                  'Aquí aparecerán el pagador, archivo, estado y acciones de revisión.',
                              icon: Icons.receipt_long_outlined,
                            )
                          : SingleChildScrollView(
                              child: _proofInspector(selectedProof!),
                            ),
                    ),
                  ],
                ),
  ...
  Widget _proofRow(Map<String, dynamic> wrapper, {required bool desktop}) {
    ...
    return VitiEntityRow(
      title:
          '${_money(payment['monto'])} · ${type == 'suscripcion' ? 'Suscripción' : _text(parent['codigo'], 'Proyecto')}',
      subtitle:
          '${_text(company['nombre_comercial'], 'Sin empresa')} · ${_text(payer['nombre'], _text(payer['usuario'], 'Pagador'))}\n${_pretty(payment['metodo'])} · ${_date(payment['fecha_pago'])}',
      icon: _fileIcon(
        _text(payment['comprobante_mime'], ''),
        _text(payment['comprobante_nombre'], ''),
      ),
      selected:
          selectedProof != null &&
          _int(_map(selectedProof!['pago'])['id']) == _int(payment['id']) &&
          _text(selectedProof!['tipo'], '') == type,
      badges: [
        VitiStatusBadge(vitiPretty(status), tone: vitiToneForStatus(status)),
        VitiStatusBadge(
          type == 'suscripcion' ? 'Suscripción' : 'Proyecto',
          tone: VitiTone.info,
        ),
      ],
      onTap: () {
  ...
  Widget _proofInspector(Map<String, dynamic> wrapper) {
    ...
    return VitiInspector(
      ...
      actions: [
        OutlinedButton.icon(
          onPressed: () => _showProof(wrapper),
          icon: const Icon(Icons.open_in_new),
          label: const Text('Abrir ficha'),
        ),
        if (pending)
          TextButton.icon(
            onPressed: () => _rejectProof(wrapper),
            icon: const Icon(Icons.close),
            label: const Text('Rechazar'),
          ),
        if (pending)
          FilledButton.icon(
            onPressed: () => _confirmProof(wrapper),
      ...
      children: [
        Container(
          ...
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 8),
              Text(
                _text(payment['comprobante_mime'], 'Archivo'),
                style: TextStyle(
                  color: Theme.of(context).colorScheme.onSurfaceVariant,
                ),
              ),
              ...
              const Text(
                'Vista integrada del archivo: siguiente etapa',
        ...
        VitiKeyValue(
          'Pagador',
          _text(payer['nombre'], _text(payer['usuario'], 'No identificado')),
        ...
        VitiKeyValue(
          type == 'suscripcion' ? 'Plan' : 'Proyecto',
          type == 'suscripcion'
              ? _text(parent['plan'], 'Suscripción VITI')
              : '${_text(parent['codigo'], 'PRO')} · ${_text(parent['nombre'], 'Proyecto')}',
          icon: type == 'suscripcion'
              ? Icons.autorenew
        ...
        children: [
          ...
          for (final project in projects.take(12))
            ListTile(
              ...
              subtitle: Text(
                '${_text(_map(project['empresa'])['nombre_comercial'], 'Sin empresa')} · Pendiente ${_money(project['pendiente'])}',
              ),
              trailing: VitiStatusBadge(
                vitiPretty('${project['estado_pago'] ?? ''}'),
                tone: vitiToneForStatus('${project['estado_pago'] ?? ''}'),
              ),
            ),
        ],
      ),
      ...
      builder: (dialogContext) => AlertDialog(
        title: Text(
          type == 'suscripcion'
              ...
              : 'Comprobante · ${_text(parent['codigo'], 'Proyecto')}',
        ),
        content: SizedBox(
          width: 700,
          child: VitiPanel(
              ...
              children: [
                ...
                SizedBox(
                  ...
                  child: VitiKeyValue(
                    ...
                    _text(
                      payer['nombre'],
                      _text(payer['usuario'], 'No identificado'),
                    ),
                    icon: Icons.person_outline,
                  ),
                ),
                SizedBox(
                  ...
                  child: VitiKeyValue(
                    'Archivo',
                    _text(
                      payment['comprobante_nombre'],
                      'Sin archivo identificado',
                    ),
                    icon: _fileIcon(
                      _text(payment['comprobante_mime'], ''),
                      _text(payment['comprobante_nombre'], ''),
  ...
  Future<void> _confirmProof(Map<String, dynamic> wrapper) async {
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text(
            'Comprobante confirmado. Los saldos fueron actualizados.',
          ),
        ),
      );
      await _load();
  ...
  Future<void> _rejectProof(Map<String, dynamic> wrapper) async {
        ...
        content: SizedBox(
          ...
          child: TextField(
            ...
            decoration: const InputDecoration(
              labelText: 'Motivo para el cliente *',
            ),
          ),
        ),
        actions: [
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text(
            'Comprobante rechazado. El cliente podrá enviar uno nuevo.',
          ),
        ),
      );
      await _load();
  ...
  List<Map<String, dynamic>> _collectProofs(
    ...
    for (final subscription in subscriptions) {
      for (final payment in _items(subscription['pagos'])) {
        items.add(<String, dynamic>{
          'tipo': 'suscripcion',
          'padre': subscription,
          'empresa': _map(subscription['empresa']),
          'pago': payment,
        });
      }
    }
    items.sort((a, b) {
      final ad =
          DateTime.tryParse(
            '${_map(a['pago'])['created_at'] ?? _map(a['pago'])['fecha_pago'] ?? ''}',
          ) ??
          DateTime.fromMillisecondsSinceEpoch(0);
      final bd =
          DateTime.tryParse(
            '${_map(b['pago'])['created_at'] ?? _map(b['pago'])['fecha_pago'] ?? ''}',
          ) ??
          DateTime.fromMillisecondsSinceEpoch(0);
