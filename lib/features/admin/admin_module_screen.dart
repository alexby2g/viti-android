...
class _AdminModuleScreenState extends State<AdminModuleScreen> {
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      if (widget.module != 'inicio' && selected != null) {
        ...
        selected = rows
            .where((row) => _int(row['id']) == selectedId)
            .firstOrNull;
      }
    ...
    return Column(
      ...
      children: [
        VitiPageHeader(
          ...
          subtitle:
              'Una vista ejecutiva de solicitudes, desarrollo, entregas y soporte. Entra al dato y continúa el trabajo sin abandonar el contexto.',
          actions: [
        ...
        Wrap(
          ...
          children: [
            VitiMetricTile(
              label: 'Negocios',
              value: '${summary['negocios_activos'] ?? 0}',
              icon: Icons.business_outlined,
              tone: VitiTone.info,
              onTap: () => widget.onNavigate('empresas'),
            ),
            VitiMetricTile(
              label: 'Solicitudes activas',
              value: '${summary['solicitudes_activas'] ?? 0}',
              icon: Icons.assignment_outlined,
              tone: VitiTone.warning,
              onTap: () => widget.onNavigate('solicitudes'),
            ),
            VitiMetricTile(
              label: 'Proyectos activos',
              value: '${summary['proyectos_activos'] ?? 0}',
              icon: Icons.account_tree_outlined,
              tone: VitiTone.primary,
              onTap: () => widget.onNavigate('proyectos'),
            ),
            VitiMetricTile(
              label: 'Apps activas',
              value: '${summary['aplicaciones_activas'] ?? 0}',
              icon: Icons.apps_outlined,
              tone: VitiTone.success,
              onTap: () => widget.onNavigate('aplicaciones'),
            ),
            VitiMetricTile(
              label: 'Pendientes de entrega',
              value: '${summary['aplicaciones_pendientes_entrega'] ?? 0}',
            ...
            VitiMetricTile(
              ...
              value: '${summary['mantenimientos_abiertos'] ?? 0}',
              icon: Icons.support_agent_outlined,
              tone: VitiTone.info,
              onTap: widget.superadmin
                  ? () => widget.onNavigate('mensajes')
                  : null,
            ),
          ],
        ),
        ...
        LayoutBuilder(
          builder: (context, constraints) {
            ...
            return Row(
              ...
              children: [
                Expanded(child: requestBlock),
                const SizedBox(width: 14),
                Expanded(child: projectBlock),
              ],
            );
          },
        ),
    ...
    return VitiPanel(
      child: Column(
        ...
        children: [
          ...
          Text(
            'Los accesos más frecuentes de operación VITI.',
            style: TextStyle(
          ...
          Wrap(
            ...
            children: [
              FilledButton.tonalIcon(
                onPressed: () => widget.onNavigate('solicitudes'),
                icon: const Icon(Icons.fact_check_outlined),
                label: const Text('Revisar solicitudes'),
              ),
              FilledButton.tonalIcon(
                onPressed: () => widget.onNavigate('proyectos'),
                icon: const Icon(Icons.account_tree_outlined),
                label: const Text('Gestionar proyectos'),
              ),
              FilledButton.tonalIcon(
                onPressed: () => widget.onNavigate('aplicaciones'),
                icon: const Icon(Icons.apps_outlined),
                label: const Text('Controlar aplicaciones'),
              ),
              if (widget.superadmin)
                FilledButton.tonalIcon(
                  onPressed: () => widget.onNavigate('pagos'),
                  icon: const Icon(Icons.receipt_long_outlined),
                  label: const Text('Revisar pagos'),
                ),
              if (widget.superadmin)
                FilledButton.tonalIcon(
                  onPressed: () => widget.onNavigate('mensajes'),
                  icon: const Icon(Icons.forum_outlined),
                  label: const Text('Atender mensajes'),
                ),
              OutlinedButton.icon(
                onPressed: () => widget.onNavigate('guia'),
                icon: const Icon(Icons.route_outlined),
                label: const Text('Guía VITI'),
              ),
            ],
          ),
        ],
    ...
    return VitiPanel(
      child: Column(
        ...
        children: [
          ...
          if (items.isEmpty)
            const VitiEmptyState(
              title: 'Sin actividad reciente',
              message: 'Los nuevos movimientos aparecerán aquí.',
            ),
  ...
  Widget _collectionExperience(List<Map<String, dynamic>> source) {
    ...
    final filtered = source
        .where((row) {
          final haystack =
              '${_rowTitle(meta.kind, row)} ${_rowSubtitle(meta.kind, row)}'
                  .toLowerCase();
          final matchesQuery =
              query.trim().isEmpty ||
          ...
          final matchesStatus =
              statusFilter == 'todos' || rowStatus == statusFilter;
          return matchesQuery && matchesStatus;
        })
        .toList(growable: false);

    final statuses = <String>{
      for (final row in source) _statusFor(meta.kind, row),
        ...
        final content = Column(
          ...
          children: [
            VitiPageHeader(
              title: meta.title,
              subtitle:
                  '${meta.subtitle} · ${source.length} registrados · ${filtered.length} visibles',
              actions: [
                IconButton.filledTonal(
                  onPressed: _load,
                  tooltip: 'Actualizar',
            ...
            if (filtered.isEmpty)
              const VitiEmptyState(
                ...
                message:
                    'Ajusta la búsqueda o los filtros para encontrar registros.',
              )
            else if (desktop)
              SizedBox(
                ...
                child: Row(
                  ...
                  children: [
                    Expanded(
                      flex: 6,
                      child: _entityList(meta.kind, filtered, scrollable: true),
                    ),
                    const SizedBox(width: 14),
                    Expanded(
                      flex: 4,
    ...
    final body = ListView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        return VitiEntityRow(
          ...
          icon: _kindIcon(kind),
          selected:
              selected != null && _int(selected!['id']) == _int(row['id']),
          badges: _rowBadges(kind, row),
          onTap: () {
  ...
  Widget _emptyInspector(_ModuleMeta meta) {
    return VitiEmptyState(
      ...
      message:
          'La ficha rápida aparecerá aquí sin cubrir la lista. Desde ella puedes abrir el detalle y continuar el trabajo.',
      icon: meta.icon,
    );
  }
  ...
  Widget _inspector(String kind, Map<String, dynamic> row) {
    ...
    final fields = switch (kind) {
      'empresa' => <(String, String, IconData)>[
        ...
        (
          'Actividad',
          _text(row['actividad'], 'No definida'),
          Icons.work_outline,
        ),
        (
          'Responsable',
          _text(_map(row['cliente'])['nombre'], 'Sin responsable'),
      ...
      'solicitud' => <(String, String, IconData)>[
        ...
        (
          'Empresa',
          _text(company['nombre_comercial'], 'Sin empresa'),
          Icons.business_outlined,
        ),
        (
          'Cliente',
          _text(client['nombre'], 'Sin cliente'),
          Icons.person_outline,
        ),
        (
          'Prioridad',
          vitiPretty('${row['prioridad'] ?? 'normal'}'),
          Icons.flag_outlined,
        ),
      ],
      ...
      'aplicacion' => <(String, String, IconData)>[
        ...
        (
          'Entorno',
          vitiPretty('${row['entorno'] ?? ''}'),
          Icons.cloud_outlined,
        ),
        (
          'Versión',
          _text(row['version'], 'Sin versión'),
          Icons.commit_outlined,
        ),
        (
          'Acceso',
          row['acceso_cliente'] == true ? 'Entregado' : 'Pendiente',
          Icons.key_outlined,
        ),
      ],
    ...
    return VitiInspector(
      ...
      actions: [
        FilledButton.icon(
          onPressed: () => _openDetails(kind, row),
          icon: const Icon(Icons.open_in_new),
          label: const Text('Abrir detalle'),
        ),
        if (kind == 'proyecto')
          OutlinedButton.icon(
            onPressed: () => _showAddProgress(row),
        ...
        if (kind == 'aplicacion')
          OutlinedButton.icon(
            ...
            label: const Text('Operar app'),
          ),
      ],
      children: [
        for (final field in fields)
          VitiKeyValue(field.$1, field.$2, icon: field.$3),
        if (kind == 'proyecto') ...[
          const SizedBox(height: 4),
          ClipRRect(
            borderRadius: BorderRadius.circular(99),
            child: LinearProgressIndicator(
              value: (_int(row['progreso']).clamp(0, 100)) / 100,
              minHeight: 7,
            ),
          ),
        ],
                      ...
                      children: [
                        ...
                        Expanded(
                          child: Column(
                            ...
                            children: [
                              Text(
                                _text(app['nombre'], 'Aplicación VITI'),
                              ...
                              Wrap(
                                ...
                                children: [
                                  VitiStatusBadge(
                                    vitiPretty('${app['entorno']}'),
                                    tone: vitiToneForStatus(
                                      '${app['entorno']}',
                                    ),
                                  ),
                                  VitiStatusBadge(
                                    vitiPretty('${app['estado']}'),
                                    tone: vitiToneForStatus('${app['estado']}'),
                                  ),
                                  VitiStatusBadge(
                                    app['acceso_cliente'] == true
                                        ? 'Entregada'
                                        : 'Entrega pendiente',
                                    tone: app['acceso_cliente'] == true
                                        ? VitiTone.success
                                        : VitiTone.warning,
                                  ),
                                ],
                              ...
                              child: Column(
                                ...
                                children: [
                                  ...
                                  VitiKeyValue(
                                    ...
                                    _text(
                                      project['nombre'],
                                      _text(project['codigo'], 'Sin proyecto'),
                                    ),
                                    icon: Icons.account_tree_outlined,
                                  ),
                                ],
                            ...
                            final operation = VitiPanel(
                              child: Column(
                                ...
                                children: [
                                  ...
                                  const SizedBox(height: 5),
                                  Text(
                                    'Controla ciclo, acceso y entra al sistema sin abandonar VITI.',
                                    style: TextStyle(
                                      fontSize: 12,
                                      color: Theme.of(
                                        context,
                                  ...
                                  Wrap(
                                    ...
                                    children: [
                                      ...
                                      OutlinedButton.icon(
                                        onPressed: () async {
                                          final cycle =
                                              await _showAppCycleDialog(app);
                                          if (cycle == null) return;
                                          await perform(() async {
                                            app = await widget.repository
                                                .updateAdminAppCycle(
                                                  appId: _int(app['id']),
                                                  environment:
                                                      '${cycle['entorno']}',
                                  ...
                                  if (!native) ...[
                                    const SizedBox(height: 12),
                                    Text(
                                      'El control administrativo está disponible; esta app todavía no tiene workspace nativo Flutter.',
                                      style: TextStyle(
                                        color: Theme.of(
                                          context,
                                        ).colorScheme.onSurfaceVariant,
  ...
  Future<Map<String, dynamic>?> _showAppCycleDialog(
    Map<String, dynamic> app,
  ) async {
    var environment =
        const {'desarrollo', 'beta', 'produccion'}.contains('${app['entorno']}')
        ? '${app['entorno']}'
        : 'desarrollo';
    var state =
        const {
            ...
            child: Column(
              ...
              children: [
                ...
                DropdownButtonFormField<String>(
                  ...
                  onChanged: (value) =>
                      setDialogState(() => state = value ?? state),
                ),
              ],
            ),
          ),
  ...
  Future<void> _showAddProgress(Map<String, dynamic> project) async {
    ...
    var phase = phases.contains(project['fase'])
        ? '${project['fase']}'
        : 'desarrollo';
    var area = 'general';
    ...
    await showDialog<void>(
      ...
      builder: (dialogContext) => StatefulBuilder(
        builder: (context, setDialogState) => AlertDialog(
          title: Text(
            'Registrar avance · ${_text(project['codigo'], 'Proyecto')}',
          ),
          content: SizedBox(
            width: 580,
                      ...
                      Expanded(
                        child: DropdownButtonFormField<String>(
                          ...
                          onChanged: saving
                              ? null
                              : (value) =>
                                    setDialogState(() => area = value ?? area),
                        ),
                      ),
                    ],
                  ),
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    const SizedBox(height: 5),
                    Wrap(spacing: 6, children: _rowBadges(kind, data)),
                  ],
                ),
              ),
              IconButton(
                ...
                child: Wrap(
                  ...
                  children: [
                    for (final field in fields)
                      SizedBox(
                        width: 220,
                        child: VitiKeyValue(field.$1, field.$2),
                      ),
                  ],
                ),
              ),
                ...
                VitiPanel(
                  child: Column(
                    ...
                    children: [
                      ...
                      if (_items(data['avances']).isEmpty)
                        Text(
                      ...
                      for (final advance in _items(
                        data['avances'],
                      ).reversed.take(8))
                        ListTile(
                          contentPadding: EdgeInsets.zero,
                          leading: CircleAvatar(
                            child: Text('${advance['progreso'] ?? 0}%'),
                          ),
                          title: Text(
                            _text(advance['titulo'], 'Avance'),
                            style: const TextStyle(fontWeight: FontWeight.w800),
                          ),
                          subtitle: Text(
                            '${vitiPretty('${advance['fase']}')} · ${_text(advance['descripcion'], 'Sin descripción')}',
                          ),
                        ),
                    ],
                  ),
...
_ModuleMeta _metaFor(String module) => switch (module) {
  'empresas' => const _ModuleMeta(
    'Empresas',
    'Negocios, responsables y estado de relación dentro de VITI.',
    'empresa',
    Icons.business_outlined,
  ),
  'solicitudes' => const _ModuleMeta(
    'Solicitudes',
    'Entrada de nuevos sistemas, prioridades y avance de análisis.',
    'solicitud',
    Icons.assignment_outlined,
  ),
  'proyectos' => const _ModuleMeta(
    'Proyectos',
    'Desarrollo, progreso, fase y responsables.',
    'proyecto',
    Icons.account_tree_outlined,
  ),
  'aplicaciones' => const _ModuleMeta(
    'Aplicaciones',
    'Ciclo técnico, operación, acceso y entrega.',
    'aplicacion',
    Icons.apps_outlined,
  ),
  _ => const _ModuleMeta(
...
String _rowTitle(String kind, Map<String, dynamic> row) => switch (kind) {
  ...
  'solicitud' =>
    '${_text(row['codigo'], 'SOL')} · ${_text(row['titulo'], 'Solicitud')}',
  'proyecto' =>
    '${_text(row['codigo'], 'PRO')} · ${_text(row['nombre'], 'Proyecto')}',
...
String _rowSubtitle(String kind, Map<String, dynamic> row) {
  ...
  return switch (kind) {
    'empresa' =>
      '${_text(row['codigo'], 'Sin código')} · ${_text(row['actividad'], 'Actividad por definir')}\n${_text(client['nombre'], 'Sin responsable')}',
    'solicitud' =>
      '${_text(company['nombre_comercial'], 'Empresa por definir')} · ${_text(client['nombre'], 'Sin cliente')}\nPrioridad ${vitiPretty('${row['prioridad'] ?? 'normal'}')}',
    'proyecto' =>
      '${_text(company['nombre_comercial'], 'Sin empresa')} · ${vitiPretty('${row['fase'] ?? ''}')}\n${row['progreso'] ?? 0}% de progreso',
    'aplicacion' =>
      '${_text(company['nombre_comercial'], 'Sin empresa')} · ${vitiPretty('${row['entorno'] ?? ''}')}\n${row['acceso_cliente'] == true ? 'Acceso entregado' : 'Entrega pendiente'}',
    _ => '',
  };
}
...
List<Widget> _rowBadges(String kind, Map<String, dynamic> row) {
  ...
  if (kind == 'solicitud') {
    ...
    widgets.add(
      VitiStatusBadge(
        'Prioridad ${vitiPretty(priority)}',
        tone: priority == 'alta' || priority == 'urgente'
            ? VitiTone.warning
            : VitiTone.neutral,
      ),
    );
  ...
  if (kind == 'proyecto') {
    widgets.add(
      VitiStatusBadge('${row['progreso'] ?? 0}%', tone: VitiTone.primary),
    );
  }
  if (kind == 'aplicacion') {
    ...
    widgets.add(
      VitiStatusBadge(
        vitiPretty(environment),
        tone: vitiToneForStatus(environment),
      ),
    );
    widgets.add(
      VitiStatusBadge(
        row['acceso_cliente'] == true ? 'Entregada' : 'Pendiente entrega',
        tone: row['acceso_cliente'] == true
            ? VitiTone.success
            : VitiTone.warning,
      ),
    );
...
List<(String, String)> _detailFields(String kind, Map<String, dynamic> data) {
  ...
  return switch (kind) {
    'empresa' => [
      ...
      (
        'Teléfono',
        _text(client['telefono'], _text(data['telefono'], 'No registrado')),
      ),
