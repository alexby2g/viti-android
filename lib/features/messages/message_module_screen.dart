...
class _MessageModuleScreenState extends State<MessageModuleScreen> {
  ...
  Future<List<Map<String, dynamic>>> _fetchInbox() {
    ...
    return widget.admin
        ? widget.repository.adminInbox()
        : widget.repository.clientInbox();
  }

  Future<Map<String, dynamic>> _fetchConversation(int id) {
    if (widget.support) return widget.repository.supportConversation(id);
    return widget.admin
        ? widget.repository.adminConversation(id)
        : widget.repository.clientConversation(id);
  }
  ...
  Future<void> _loadInbox() async {
    ...
    try {
      ...
      rows = [...rows]
        ..sort((a, b) => _latestDate(b).compareTo(_latestDate(a)));
      if (selectedId != null &&
          !rows.any((row) => _int(row['id']) == selectedId)) {
        selectedId = null;
        current = null;
      }
  ...
  Future<void> _attach() async {
    ...
    try {
      final message = reply.text.trim();
      if (widget.support) {
        await widget.repository.sendSupportFile(
          selectedId!,
          filePath: file.path!,
          fileName: file.name,
          message: message,
        ...
        if (image) {
          await widget.repository.sendAdminImage(
            selectedId!,
            filePath: file.path!,
            fileName: file.name,
            message: message,
          ...
          await widget.repository.sendAdminDocument(
            selectedId!,
            filePath: file.path!,
            fileName: file.name,
            message: message,
        ...
        if (image) {
          await widget.repository.sendClientImage(
            selectedId!,
            filePath: file.path!,
            fileName: file.name,
            message: message,
          ...
          await widget.repository.sendClientDocument(
            selectedId!,
            filePath: file.path!,
            fileName: file.name,
            message: message,
          ...
          return Padding(
            ...
            child: Row(
              children: [
                SizedBox(
                  width: 350,
                  child: VitiPanel(padding: EdgeInsets.zero, child: _inbox()),
                ),
  ...
  String get _title => widget.support
      ? 'Mensajes asignados'
      : widget.admin
      ? 'Atención VITI'
      : 'Mi buzón';
  String get _subtitle => widget.support
      ? 'Solo conversaciones delegadas a tu cuenta.'
      : widget.admin
      ? 'Conversaciones privadas autorizadas.'
      : 'Mensajes y documentos con AGR Studio.';
  ...
  Widget _inbox() {
    final visible = rows
        .where((row) {
          ...
          final haystack =
              '${_contactName(row)} ${row['asunto']} ${_preview(row)}'
                  .toLowerCase();
        ...
        children: [
          ...
          if (visible.isEmpty)
            const VitiEmptyState(
              ...
              message: 'Las conversaciones disponibles aparecerán aquí.',
  ...
  Widget _conversation({bool mobile = false}) {
    final colors = Theme.of(context).colorScheme;
    if (selectedId == null) {
      return const VitiEmptyState(
        title: 'Selecciona una conversación',
        message: 'Los mensajes aparecerán aquí sin cubrir tu bandeja.',
        icon: Icons.chat_bubble_outline,
      );
    }
    if (loadingConversation && current == null) {
      return const Center(child: CircularProgressIndicator());
    }
            ...
            children: [
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    Text(
                      _contactName(conversation),
                      style: const TextStyle(fontWeight: FontWeight.w900),
                    ),
                    const SizedBox(height: 2),
                    Text(
                      _text(conversation['asunto'], 'Conversación VITI'),
                      style: TextStyle(
                        fontSize: 11,
                        color: colors.onSurfaceVariant,
                      ),
              ...
              VitiStatusBadge(
                widget.support ? 'Asignada' : 'Privada',
                tone: VitiTone.success,
              ),
              const SizedBox(width: 6),
              IconButton(
                onPressed: loadingConversation
                    ? null
                    : () => _select(selectedId!),
                tooltip: 'Actualizar',
                icon: const Icon(Icons.refresh),
      ...
      child: Container(
        ...
        child: Column(
          ...
          children: [
            ...
            if (fileName.isNotEmpty && text.isNotEmpty)
              const SizedBox(height: 7),
            if (text.isNotEmpty)
              Text(text, style: const TextStyle(height: 1.35)),
            if (text.isEmpty && fileName.isEmpty)
              Text('Adjunto', style: TextStyle(color: colors.onSurfaceVariant)),
            const SizedBox(height: 5),
            Text(
...
String _initial(String text) =>
    text.trim().isEmpty ? 'V' : text.trim().characters.first.toUpperCase();
String _contactName(Map<String, dynamic> row) {
  ...
  return _text(
    contact['nombre'],
    _text(client['nombre'], _text(company['nombre_comercial'], 'Equipo VITI')),
  );
}
...
DateTime _messageDate(Map<String, dynamic> message) =>
    DateTime.tryParse(
      '${message['created_at'] ?? message['enviado_at'] ?? ''}',
    )?.toLocal() ??
...
DateTime _latestDate(Map<String, dynamic> row) {
  ...
  return DateTime.tryParse(
        '${row['ultimo_mensaje_at'] ?? row['updated_at'] ?? ''}',
      )?.toLocal() ??
      DateTime.fromMillisecondsSinceEpoch(0);
}

String _time(dynamic value) {
  final DateTime? parsed = value is DateTime
      ? value
