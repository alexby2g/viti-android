...
/// Lenguaje visual compartido entre las pantallas nativas de VITI.
/// Mantiene densidad de escritorio, jerarquía clara y colores semánticos.
...
class VitiPanel extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final fill = selected
        ? Color.alphaBlend(toneColor.withValues(alpha: .10), colors.surface)
        : colors.surface;
    final border = selected
        ? toneColor.withValues(alpha: .54)
    ...
    final content = Container(
      decoration: BoxDecoration(
        ...
        boxShadow: selected
            ? [BoxShadow(color: toneColor.withValues(alpha: .08), blurRadius: 22, offset: const Offset(0, 8))]
            : const [],
      ),
        ...
        child: Row(
          ...
          children: [
            Container(
              ...
              height: 40,
              decoration: BoxDecoration(
                color: toneColor.withValues(alpha: .13),
              ...
              child: Column(
                ...
                children: [
                  ...
                  if (caption != null) ...[
                    ...
                    Text(caption!, style: TextStyle(fontSize: 11, color: colors.onSurfaceVariant)),
                  ],
                ],
              ),
            ),
        ...
        child: InkWell(
          ...
          child: Container(
            ...
            decoration: BoxDecoration(
              borderRadius: BorderRadius.circular(14),
              border: Border.all(
                color: selected ? colors.primary.withValues(alpha: .54) : colors.outlineVariant.withValues(alpha: .65),
              ),
            ),
            child: Row(
              children: [
                Container(
                  ...
                  height: 38,
                  decoration: BoxDecoration(
                    color: selected ? colors.primary.withValues(alpha: .15) : colors.surfaceContainerHighest,
                ...
                Expanded(
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Text(title, maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(fontWeight: FontWeight.w850)),
                      const SizedBox(height: 3),
...
VitiTone vitiToneForStatus(String? raw) {
  ...
  if ({'activo', 'aprobada', 'aprobado', 'pagado', 'confirmado', 'entregado', 'entregada', 'finalizado', 'finalizada', 'produccion', 'resuelto', 'cerrado'}.contains(value)) {
    return VitiTone.success;
  }
  if ({'borrador', 'pendiente', 'pendiente_revision', 'pendiente_anticipo', 'en_revision', 'en_espera', 'beta', 'en_pruebas'}.contains(value)) {
    return VitiTone.warning;
  }
  if ({'rechazado', 'rechazada', 'cancelado', 'cancelada', 'retirado', 'fallido', 'vencido'}.contains(value)) {
    return VitiTone.danger;
  }
  if ({'convertida', 'desarrollo', 'analisis', 'diseno', 'implementacion', 'mantenimiento'}.contains(value)) {
    return VitiTone.primary;
  }
